...
export class StorageService {
  ...
  async upload(input: UploadInput) {
    ...
    const bucket = this.safeSegment(input.bucket || this.configService.get<string>('SUPABASE_DEFAULT_BUCKET', 'presfm'));
    const objectPath = this.normalizeObjectPath(input.path || `${Date.now()}-${input.file.originalname}`);
    ...
    if (provider === 'supabase') {
      await this.uploadToSupabase(bucket, objectPath, input.file.buffer, input.file.mimetype || 'application/octet-stream');
    } else {
    ...
    const result = await this.databaseService.query<StoredFile>(
      ...
      [provider, bucket, objectPath, input.file.originalname || null, input.file.mimetype || null, input.file.size, checksum, input.uploadedBy || null, input.projectId || null, input.datasetId || null],
    );
  ...
  async signedUrl(bucket: string, objectPath: string, expiresIn = 3600) {
    ...
    if (!this.isSupabaseEnabled()) {
      return { signedUrl: null, provider: 'local', bucket: safeBucket, path: safePath };
    }
    const response = await fetch(`${this.supabaseUrl()}/storage/v1/object/sign/${safeBucket}/${safePath}`, {
      method: 'POST',
    ...
    return { signedUrl: url ? `${this.supabaseUrl()}${url}` : null, provider: 'supabase', bucket: safeBucket, path: safePath };
  }

  async remove(id: string) {
    const file = await this.findById(id);
    if (file.provider === 'supabase') {
      await fetch(`${this.supabaseUrl()}/storage/v1/object/${file.bucket}/${file.objectPath}`, {
        method: 'DELETE',
        headers: this.supabaseJsonHeaders(),
      });
    } else {
    ...
    await this.databaseService.query(`DELETE FROM storage_files WHERE id = $1`, [id]);
    return { success: true };
  }

  private async uploadToSupabase(bucket: string, objectPath: string, buffer: Buffer, contentType: string) {
  ...
  private async downloadFromSupabase(bucket: string, objectPath: string) {
    const response = await fetch(`${this.supabaseUrl()}/storage/v1/object/authenticated/${bucket}/${objectPath}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${this.supabaseKey()}`,
        apikey: this.supabaseKey(),
  ...
  private async uploadToLocal(bucket: string, objectPath: string, buffer: Buffer) {
  ...
  private localPath(bucket: string, objectPath: string) {
    return join(this.configService.get<string>('LOCAL_STORAGE_ROOT', '/data/presfm-storage'), bucket, objectPath);
  }

  private isSupabaseEnabled() {
    return Boolean(this.configService.get<string>('SUPABASE_URL') && this.configService.get<string>('SUPABASE_SERVICE_ROLE_KEY'));
  }
